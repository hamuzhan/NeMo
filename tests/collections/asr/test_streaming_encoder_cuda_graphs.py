...
class TestStreamingEncoderCudaGraphsGPU:
    ...
    def test_graphed_stream_step_bit_exact(self, batch_size: int):
        """Graphed steady-state steps must be bit-exact with eager, including the cache-fill
        phase and the final keep_all_outputs=True step (eager fallback)."""
        device = "cuda"
