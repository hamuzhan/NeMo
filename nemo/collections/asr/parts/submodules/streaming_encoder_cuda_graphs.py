...
"""CUDA-graph acceleration for the cache-aware streaming encoder step.

The cache-aware streaming encoder (see :class:`~nemo.collections.asr.parts.mixins.streaming.StreamingEncoder`)
issues on the order of a thousand small kernel launches per streaming step, which makes low-latency
streaming inference host-bound: the GPU is idle most of the step waiting for kernel launches.
Since the steady-state streaming step has fully static shapes — the cache tensors are allocated at
their full size up front and only ``cache_last_channel_len`` values change — the whole step can be
captured once into a :class:`torch.cuda.CUDAGraph` and replayed with a single launch.

This mirrors the design of the RNNT/TDT label-looping decoders
(:class:`~nemo.collections.asr.parts.submodules.transducer_decoding.label_looping_base.GreedyBatchedLabelLoopingComputerBase`),
with two differences: the encoder has no data-dependent control flow, so plain CUDA graphs are
sufficient (no conditional nodes, no ``cuda-python`` requirement), and outputs are written into
stable buffers owned by the regular caching allocator so that no consumer (e.g. the decoder's own
CUDA graph) can alias the encoder graph's private memory pool.

Non-uniform steps fall back to eager execution automatically: the first step(s) (different
pre-encode cache handling), the final step of an utterance (``keep_all_outputs=True``), and any
call whose shapes/parameters do not match an already-captured graph.
"""
